            ...
            <div>
              ...
              {cartItems.length > 0 ? (
                <div className="space-y-4">
                  {cartItems.map((item) => (
                    <div
                      key={item.id}
                      className="grid grid-cols-1 gap-4 rounded-xl border border-slate-200 bg-white p-3 sm:grid-cols-[auto_minmax(0,1fr)_auto_auto_auto] sm:items-center"
                    >
              ...
              <div className="mt-10">
                <div className="mb-4 flex items-center justify-between">
                  <h2 className="text-xs font-bold uppercase tracking-[0.2em] text-description">You Might Also Like</h2>
                  <button
                    type="button"
                ...
                >
                  {youMayAlsoLike.map((product) => (
                    <article key={product.id} className="min-w-45 rounded-lg border border-slate-200 bg-white p-2">
                      ...
                      <h3 className="mt-2 line-clamp-1 text-xs font-semibold uppercase text-title">{product.name}</h3>
                      <div className="mt-1 flex items-center gap-1 text-xs">
                        <span className="font-bold text-heading">{formatMoney(product.currentPrice)}</span>
                        <span className="text-slate-400 line-through">{formatMoney(product.oldPrice)}</span>
                        <span className="rounded bg-heading px-1 py-0.5 text-[10px] text-white">-{product.discount}</span>
                      </div>
                      <button
                        type="button"
                        className="mt-2 inline-flex w-full items-center justify-center rounded border border-slate-300 px-2 py-1.5 text-[11px] font-semibold text-title transition hover:border-slate-400"
                      >
                        Add to cart
                      </button>
                    </article>
            ...
            <aside className="h-fit rounded-xl border border-slate-200 bg-white p-5 shadow-sm lg:sticky lg:top-24">
              ...
              <div className="mt-4 border-t border-slate-200 pt-4">
                ...
                <button
                  ...
                  className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-md bg-heading px-4 py-3 text-sm font-semibold text-white transition hover:opacity-95 disabled:cursor-not-allowed disabled:bg-slate-300"
                >
